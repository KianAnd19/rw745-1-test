...
folder_path = Path('test_suite/alan-2022-examples/passing/')
files_passing = [item for item in folder_path.iterdir() if item.is_file()]
files_passing.sort()

folder_path = Path('test_suite/alan-2022-examples/failing/')
files_failing = [item for item in folder_path.iterdir() if item.is_file()]
files_failing.sort()
results = {}

def run_passing(filename):
    input_file = f'./test_suite/alan-2022-examples/passing/{filename}'
    result = subprocess.run(['java', 'Alan', input_file], capture_output=True, text=True)
...
def run_failing(filename):
    input_file = f'./test_suite/alan-2022-examples/failing/{filename}'
